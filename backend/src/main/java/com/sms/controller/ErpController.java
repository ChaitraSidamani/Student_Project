...
public class ErpController {
    ...
    public ErpController(
            ...
            SubjectComponentRepository subjectComponentRepository) {
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> saveFaculty(@RequestBody Map<String, Object> request) {
        ...
        // Auto-create department from course code/name if not yet in DB
        Department resolvedDepartment = null;
        if (departmentId != null) {
            resolvedDepartment = departmentRepository.findById(departmentId).orElse(null);
        } else if (departmentCode != null && !departmentCode.isBlank()) {
            resolvedDepartment = resolveDepartment(null, departmentCode, departmentName);
        }
        faculty.setDepartment(resolvedDepartment);
    ...
    // ─── Subject Components (DB-backed, fixes the localStorage bug) ───────────

    @GetMapping("/subject-components/{subjectId}")
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getSubjectComponents(@PathVariable Long subjectId) {
        List<Map<String, Object>> rows = subjectComponentRepository
                .findBySubjectIdAndActiveTrueOrderBySequenceAscIdAsc(subjectId)
                .stream().map(this::componentRow).toList();
        return ResponseEntity.ok(ApiResponse.success("Components fetched", rows));
    }

    @PostMapping("/subject-components/{subjectId}")
    @PreAuthorize("hasRole('ADMIN')")
    @Transactional
    public ResponseEntity<ApiResponse<Map<String, Object>>> saveSubjectComponent(
            @PathVariable Long subjectId,
            @RequestBody Map<String, Object> request) {
        Subject subject = subjectRepository.findById(subjectId)
                .orElseThrow(() -> new RuntimeException("Subject not found: " + subjectId));
        String name = text(request, "name");
        Integer maxMarks = request.get("maxMarks") == null ? null : Integer.valueOf(String.valueOf(request.get("maxMarks")));
        Integer seq = request.get("sequence") == null ? 0 : Integer.valueOf(String.valueOf(request.get("sequence")));
        if (name == null || name.isBlank()) throw new RuntimeException("Component name is required");
        if (maxMarks == null || maxMarks <= 0) throw new RuntimeException("Allocated marks must be > 0");

        // Validate total doesn't exceed 100
        int existing = subjectComponentRepository
                .findBySubjectIdAndActiveTrueOrderBySequenceAscIdAsc(subjectId)
                .stream().mapToInt(SubjectComponent::getMaxMarks).sum();
        if (existing + maxMarks > 100)
            throw new RuntimeException("Total component marks cannot exceed 100. Currently used: " + existing);

        SubjectComponent component = new SubjectComponent();
        component.setSubject(subject);
        component.setName(name);
        component.setMaxMarks(maxMarks);
        component.setSequence(seq);
        component.setActive(true);
        SubjectComponent saved = subjectComponentRepository.save(component);
        return ResponseEntity.ok(ApiResponse.success("Component saved", componentRow(saved)));
    }

    @DeleteMapping("/subject-components/{componentId}")
    @PreAuthorize("hasRole('ADMIN')")
    @Transactional
    public ResponseEntity<ApiResponse<Void>> deleteSubjectComponent(@PathVariable Long componentId) {
        SubjectComponent component = subjectComponentRepository.findById(componentId)
                .orElseThrow(() -> new RuntimeException("Component not found: " + componentId));
        component.setActive(false);
        subjectComponentRepository.save(component);
        return ResponseEntity.ok(ApiResponse.success("Component deleted", null));
    }

    private Map<String, Object> componentRow(SubjectComponent c) {
        return Map.of(
                "id", c.getId(),
                "name", c.getName(),
                "maxMarks", c.getMaxMarks(),
                "sequence", c.getSequence() == null ? 0 : c.getSequence(),
                "subjectId", c.getSubject() == null ? "" : c.getSubject().getId()
        );
    }
